...
TodayILearned = new Mongo.Collection("today_i_learned");
...
if (Meteor.isServer) {
  ...
  Meteor.methods({
    ...
    "customUsers.login": function(loginData) {
      ...
      // CRITICAL: Create or get a corresponding Meteor user for auth sync
      let meteorUserId = null;
      let loginToken = null;

      if (Meteor.isServer) {
        try {
          // Check if Meteor user already exists (by customUserId link)
          let meteorUser = Meteor.users.findOne({ "profile.customUserId": user._id });

          if (!meteorUser) {
            // Also check if a Meteor user exists with same username/email (from old auth)
            meteorUser = Meteor.users.findOne({
              $or: [{ username: user.username.toLowerCase() }, { "emails.address": user.email.toLowerCase() }]
            });

            if (meteorUser) {
              // Link existing Meteor user to custom user
              Meteor.users.update(meteorUser._id, {
                $set: {
                  "profile.customUserId": user._id,
                  "profile.name": user.profile.displayName,
                  "profile.avatar": user.profile.avatar
                }
              });
              meteorUserId = meteorUser._id;
            } else {
              // Create a new Meteor user linked to this custom user
              try {
                meteorUserId = Accounts.createUser({
                  ...
                  profile: {
                    customUserId: user._id,
                    name: user.profile.displayName,
                    avatar: user.profile.avatar
                  }
                });
              } catch (createError) {
                // If creation fails (e.g., duplicate), try to find by username/email again
                console.log("Error creating Meteor user, attempting to find existing:", createError.message);
                meteorUser = Meteor.users.findOne({
                  $or: [{ username: user.username.toLowerCase() }, { "emails.address": user.email.toLowerCase() }]
                });
                if (meteorUser) {
                  // Link it
                  Meteor.users.update(meteorUser._id, {
                    $set: { "profile.customUserId": user._id }
                  });
                  meteorUserId = meteorUser._id;
                } else {
                  // If still no user, throw error
                  throw new Meteor.Error("meteor-user-creation-failed", "Failed to create or find Meteor user");
                }
              }
            }
          } else {
          ...
          // Generate a login token for client-side Meteor.loginWithToken
          const stampedToken = Accounts._generateStampedLoginToken();
          loginToken = stampedToken.token;

          // Store the hashed token in the Meteor user
          Accounts._insertHashedLoginToken(meteorUserId, {
            hashedToken: Accounts._hashLoginToken(loginToken)
          });

          // Copy roles from custom user to Meteor user
          // Handle the "can't mix grouped and non-grouped roles" issue
          if (user.roles && user.roles.CB) {
            try {
              // First, check if user has any non-grouped roles
              const existingMeteorUser = Meteor.users.findOne(meteorUserId);
              if (existingMeteorUser && existingMeteorUser.roles) {
                // If roles is an array, they're non-grouped - need to clear them
                if (Array.isArray(existingMeteorUser.roles)) {
                  Meteor.users.update(meteorUserId, { $set: { roles: {} } });
                }
              }
              // Now set the grouped roles
              Roles.setUserRoles(meteorUserId, user.roles.CB, "CB");
            } catch (rolesError) {
              console.error("Error setting roles:", rolesError.message);
              // Don't fail login if roles sync fails
            }
          }
        } catch (error) {
          // Log error but don't fail login - custom auth still works
          console.error("Error syncing with Meteor auth:", error);
          // Return without loginToken - custom auth will still work
          return {
            userId: user._id,
      ...
      return {
        userId: user._id,
        sessionToken: sessionToken,
        loginToken: loginToken, // For Meteor.loginWithToken on client
        user: user
      };
    },
    ...
    "customUsers.getCurrentUser": function(sessionToken) {
      ...
      // Also set this.userId for publications/methods context
      if (Meteor.isServer && this.setUserId) {
        // Find or create linked Meteor user
        let meteorUser = Meteor.users.findOne({ "profile.customUserId": user._id });
        if (meteorUser) {
          this.setUserId(meteorUser._id);
        }
      }
    ...
    /**
     * Get a fresh Meteor login token for an existing session
     * Used to restore Meteor auth after page reload
     */
    "customUsers.getLoginToken": function(sessionToken) {
      ...
      // Find or get the linked Meteor user
      let meteorUser = Meteor.users.findOne({ "profile.customUserId": user._id });
      if (!meteorUser) {
    ...
    "problemReports.insert": function(reportData) {
      check(reportData, {
        ...
        priority: String,
        sessionToken: Match.Optional(String), // Support custom auth
